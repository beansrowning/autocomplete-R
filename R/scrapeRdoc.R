...
scapeRdoc<-function(package = "base", website = "http://stat.ethz.ch/R-manual/R-patched/library/"){
	...
	scrapeRoutine <- function(addr, x) {
	  stopifnot(!is.na(x))
		...
		if(length(core %>% html_nodes("pre") %>% html_text()) != 0 &&
		   any(core %>% html_nodes("h3") %>% html_text() == "Usage") &&
		   grepl(pattern = "\\(", core %>% html_node("pre") %>% html_text()) &&
		   !grepl(pattern = "\\[", core %>% html_node("pre") %>% html_text())){

			Text <- strsplit(core %>% html_node("title") %>%
			                  html_text(), split = ":")[[1]][2]
			...
			for(i in 1:length(SnippetDisp)) {
				part1 <- regmatches(SnippetDisp[i],
				                    regexpr("(?)(\\(.*)", SnippetDisp[i]),
				                    invert=TRUE)[[1]][1]
				prefix[i] <- part1
				type[i] <- ftype(substitute(part1))
				part2 <- regmatches(SnippetDisp[i],
				                    regexpr("(?)(\\(.*)",
				                    SnippetDisp[i]))
				part2a <- unlist(regmatches(part2, regexpr("(?)(\\,.*)", part2),
				                 invert=TRUE))[1]
			...
			list("text" = functionname,
			     "snippet" = SnippetList,
					 "prefix" = prefix,
					 "type" = type,
					 "displayText" = SnippetDisp,
	...
	myscraplist <- lapply(myscrap, scrapeRoutine,
		                    addr = paste0(website, package, "/html/"))
	myscraplist <- myscraplist[unlist(lapply(myscraplist,
...
text <- unlist(lapply(finallist$keywords, function(x)
               	rep(x$text, length(x$prefix))))

description <- unlist(lapply(finallist$keywords, function(x)
                      	rep(x$description, length(x$prefix))))

descriptionMoreURL<-unlist(lapply(finallist$keywords, function(x)
                           	rep(x$descriptionMoreURL, length(x$prefix))))

rightLabel<-unlist(lapply(finallist$keywords, function(x)
            	     	rep(x$rightLabel, length(x$prefix))))

leftLabel <- unlist(lapply(finallist$keywords, function(x) rep(x$leftLabel,
	                                                             length(x$prefix))))
...
for (i in 1L:length(snippet)){
	mylonglist[[i]] <- list("text" = unname(text[i]),
	                        "prefix" = unname(prefix[i]),
													"snippet" = unname(snippet[i]),
													"description" = unname(description[i]),
													"rightLabel" = unname(rightLabel[i]),
													"leftLabel" = unname(leftLabel[i]),
													"descriptionMoreURL" = unname(descriptionMoreURL[i]),
													"type" = unname(type[i]))
	}

names(mylonglist) <- lapply(mylonglist, function(x) x$text)

cat(toJSON(list("keywords" = mylonglist), auto_unbox = TRUE),
